# Full Azure prototype

The full mode now covers diagram steps 2–10 using real Azure services. Read outputs/stage-f/Prototype-Runbook.md and verification-summary.json at the project root for the verified outcome, limitations and current teardown status. Start with scripts/Start-AzureFullWorker.ps1 and scripts/Submit-DocumentFolder.ps1 while the Azure environment is provisioned. The historical sections below describe earlier checkpoints.

# BMG intake prototype

The worker supports two explicit modes. Stage B simulation uses real PDFs and SQLite with simulated services. Stage D Azure upload mode uses certificate authentication, private Key Vault and real AzCopy/Blob upload, stopping at Uploaded. No invoice or EDM integration exists. The commands below run simulation; use `scripts/Start-AzureUploadWorker.ps1` and `scripts/Submit-AzureSynthetic.ps1` for the live upload checkpoint, documented in `../outputs/stage-d/Stage-D-Runbook.md`.
...
Stage C and Stage D are complete: private connectivity, actual credential retrieval and verified AzCopy upload have been tested. The cloud spending window is active; resources are not deleted automatically. The Stage B commands and historical results above still describe simulation only.

## Stage E real classification and OCR

`scripts/Start-AzureDocumentWorker.ps1` runs the `--azure-documents` mode with an isolated `Documents\BmgPocRuntime\stage-e` runtime. It extends the real upload adapter with a custom Document Intelligence classifier and prebuilt Layout OCR, stopping at `LayoutRead`. It does not perform LLM extraction or Cosmos application writes. The implementation and live validation record are in `outputs/stage-e/Stage-E-Runbook.md` at the project root; consult its checkpoint status before a demonstration.

This mode uses PdfPig for actual PDF parsing, encryption rejection and page-count limits. The classifier threshold, model ID and durable per-runtime page budgets are configuration-bound. Its `ai_requests` SQLite table retains billable-request intent, operation IDs and successful results. Unknown POST outcomes require review rather than an automatic repeated POST. Low-confidence and Other classifications also require review. The original simulation remains useful for orchestration regression tests and is not evidence of model accuracy.
